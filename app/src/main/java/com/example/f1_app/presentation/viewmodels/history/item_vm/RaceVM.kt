...
) : ItemVm<RaceItem>(), ItemWithEvent<HistoryViewModel.Event> {
    private val events: MutableSharedFlow<HistoryViewModel.Event> = MutableSharedFlow()

    fun onRaceClick() {
        itemsScope.launch {
            events.emit(HistoryViewModel.Event.RaceClickEvent(item, position))
        }
    }
...
fun RaceVM.toRecyclerViewItem() = RecyclerViewItem(R.layout.item_history_race, this)